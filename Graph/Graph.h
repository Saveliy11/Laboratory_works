...
{
    cout << "Vertex " << startVertex << " visited" << endl;
...
void Graph<T>::BFS(T& startvertex, bool* visitedverts)
{
    if (visitedverts[this->GetVertPos(startvertex)] == false)
    {
        ...
        cout << "Vertex " << startvertex << " processed" << endl;
        visitedverts[this->GetVertPos(startvertex)] = true;
    }
    vector<T> neighbors = this->GetNbrs(startvertex);
    this->vertqueue.pop();
    for (int i = 0; i < neighbors.size(); i++)
    {
        if (!visitedverts[this->GetVertPos(neighbors[i])])
        {
            ...
            cout << "Vertex " << neighbors[i] << " processed" << endl;
        }
    }
...
int Graph<T>::Dijkstra(T& startVertex)
{
    ...
    {
        cout << "Shortest distance from summit " << startVertex << " to the top " << vertList[i] << ": " << labelList[GetVertPos(vertList[i])] << endl;
    }
...
Graph<int> makeGraph()
{
    ...
    cout << "Number of graph vertices: ";
    cin >> amountVerts; cout << endl;
    cout << "Number of graph edges: ";
    ...
    cout << "The vertex at which to start the depth-first walk: ";
    cin >> vertex; cout << endl;
    graph.DFS(vertex, visitedverts);
    cout << "The vertex at which to start the breadth-first Walk: ";
    cin >> vertex; cout << endl;
    fill(visitedverts, visitedverts + 20, false);
...
void Graph<T>::Print()
{
    if (!this->IsEmpty())
    {
        ...
        int startVertex;
        cout << "From which vertex to bypass (Dijkstra's algorithm): ";
        cin >> startVertex;
        cout << endl;
