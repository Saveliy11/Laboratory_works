...
public:
	...
	Tree* balanced(int count)
	{
		...
		double a;
		cout << "Input data to balanced tree: "; cin >> a;
...
void draw_nodes(Tree* node, RenderWindow& window)
{
	if (node != nullptr)
	{
		...
		Font font;
		font.loadFromFile("D:/Fonts/Arial.ttf");
...
int main()
{
	...
	cout << "Tree after element added: " << endl;
	draw(bush);

	bush->minimum()->remove();
	cout << "The tree after removing the minimum: " << endl;
	draw(bush);

	Tree* search_bush = bush->convert_into_search();
	cout << "Tree to BDP transformation: " << endl;
	draw(search_bush);
